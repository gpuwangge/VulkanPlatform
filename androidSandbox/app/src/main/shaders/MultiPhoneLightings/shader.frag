...
void main() {
	...
	for(int i = 0; i < LIGHT_NUM; i++){
		vec3 L = normalize(inLightVec[i]); //point from pos to light
		vec3 V = normalize(inViewVec[i]); //point from pos to camera
		vec3 R = reflect(-L, N); //calculate from L
		float distCoff = pow(inLightVec[i].x, 2) + pow(inLightVec[i].y, 2) + pow(inLightVec[i].z, 2);
		...
		outColor += vec4(ambient * inAmbientIntensity[i] + diffuse * inDiffuseIntensity[i] + specular * inSpecularIntensity[i], 0.0);
	}
}