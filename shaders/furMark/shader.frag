...
void rotate(inout vec3 p) {
    p.xy *= rot2D(sin(customUBO.u_time * 0.8) * 0.25);
    p.yz *= rot2D(sin(customUBO.u_time * 0.7) * 0.2);
}
...
vec3 render(vec2 offset) {
    ...
    if (dist < 100.0) {
        ...
        phi += sin(0.3 * rho - 0.5 * customUBO.u_time);

        float h = sin(8.0 * phi) * 0.5 + 0.5;

        vec2 st;
        st.x = 3.0 * phi / PI;
        st.y = customUBO.u_time * 0.5 + PI / (rho + 0.1 * smoothstep(0.45, 0.5, h));
