    ...
    for (const auto& resource : config["Resources"]) {
        if (resource["Texts"]) {
            for (const auto& text : resource["Texts"]) {
                ...
                sdlManager.createFonts(fontSize);
                TTF_SetFontStyle(sdlManager.m_font, TTF_STYLE_BOLD);

                SDL_Color sdlColor = { 
                    static_cast<Uint8>(color.size() > 0 ? color[0] : 255),
                ...
                SDL_Surface* textSurface = TTF_RenderText_Blended(sdlManager.m_font, ascII.c_str(), 0, sdlColor);
                ...
                for(int i = 0; i < ascII.size(); i++){
                    ...
                    glyphSurface = TTF_RenderGlyph_Blended(sdlManager.m_font, ch, {255, 255, 255, 255});
                ...
                modelManager.CreateCustomModel3D(textQuadVertices, indices3D); //model for text
            }
        }
